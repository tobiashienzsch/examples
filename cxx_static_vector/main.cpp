...
auto test_builtin_type()
{
    ...
    ASSERT(vec_0.size() == 4);
    ASSERT(std::all_of(std::begin(vec_0), std::end(vec_0),
                       [](auto val) { return val == T(); }));
    ASSERT(std::all_of(std::cbegin(vec_0), std::cend(vec_0),
                       [](auto val) { return val == T(); }));
    ...
    vec_6.resize(2);
    ASSERT(vec_6.size() == 2);
    ASSERT(std::all_of(vec_6.begin(), vec_6.end(),
                       [](auto val) { return val == T(); }));
    // grow full capacity
    vec_6.resize(4, T(0));
    ASSERT(vec_6.size() == 4);
    ASSERT(std::all_of(vec_6.cbegin(), vec_6.cend(),
                       [](auto val) { return val == T(); }));
    // same size
    vec_6.resize(4, T(0));
    ASSERT(vec_6.size() == 4);
    ...
    vec_6.resize(2, T(1));
    ASSERT(vec_6.size() == 2);
    // ASSERT(std::all_of(vec_6.cbegin(), vec_6.cend(), [](auto val) { return
    // val == T(1); }));
    ...
    ASSERT(std::all_of(cvec_9.cbegin(), cvec_9.cend(),
                       [](auto val) { return val == T(42); }));
    ...
    ASSERT(std::all_of(cvec_10.cbegin(), cvec_10.cend(),
                       [](auto val) { return val == T(42); }));
}
...
auto test_struct()
{
    ...
    // construct copy
    {
        etl::static_vector<Vertex<T>, 3> original {3,
                                                   Vertex {T(1), T(2), T(3)}};
        auto copy {original};
        ASSERT(copy.full());
