...
[[nodiscard]] static auto hit(Sphere<float> const& s, Ray<float> const& r)
    -> bool
{
...
{
    auto const sphere = Sphere<float> {Vec3<float> {0.0F, 0.0F, -1.0F}, 0.5F};
    if (hit(sphere, ray))
    {
    ...
    auto const vec  = Vec3<float> {1, 1, 1} * (1.0F - t)
                     + Vec3<float> {0.5F, 0.7F, 1.0F} * t;
