...
int main(int argc, char **argv) {
  ...
  auto rate = static_cast<unsigned>(atoi(argv[1]));
  ...
  /* Write parameters */
  if (pcm = snd_pcm_hw_params(pcm_handle, params); pcm < 0)
  ...
  for (loops = (seconds * 1000000) / tmp; loops > 0; loops--) {
    ...
    if (pcm = read(0, buff, buff_size); pcm == 0) {
      printf("Early end of file.\n");
      return 0;
    }

    if (pcm = snd_pcm_writei(pcm_handle, buff, frames); pcm == -EPIPE) {
      printf("XRUN.\n");
