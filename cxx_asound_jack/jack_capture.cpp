...
int process(jack_nframes_t nframes, void *arg) {
  ...
  for (chn = 0; chn < nports; chn++)
    in[chn] = (jack_default_audio_sample_t *)jack_port_get_buffer(ports[chn],
                                                                  nframes);
...
void run_disk_thread(jack_thread_info_t *info) {
  ...
  if (overruns > 0) {
    ...
    fprintf(stderr, " try a bigger buffer than -B %" PRIu32 ".\n",
            info->rb_size);
    info->status = EPIPE;
  }
}

void setup_ports(int sources, char const *source_names[],
  ...
  for (i = 0; i < nports; i++) {
    ...
    if ((ports[i] =
             jack_port_register(info->client, name, JACK_DEFAULT_AUDIO_TYPE,
                                JackPortIsInput, 0)) == 0) {
...

{
  ...
  int show_usage = 0;
  char const *optstring = "d:f:b:B:h";
  struct option long_options[] = {
      {"help", 0, 0, 'h'},     {"duration", 1, 0, 'd'}, {"file", 1, 0, 'f'},
      {"bitdepth", 1, 0, 'b'}, {"bufsize", 1, 0, 'B'},  {0, 0, 0, 0}};
  ...
  if (show_usage || thread_info.path == NULL || optind == argc) {
    fprintf(stderr, "usage: jackrec -f filename [ -d second ] [ -b bitdepth ] "
                    "[ -B bufsize ] port1 [ port2 ... ]\n");
    exit(1);
  }
  ...
  setup_ports(argc - optind, (const char **)&argv[optind], &thread_info);
