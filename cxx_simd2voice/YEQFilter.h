...
public:
    ...
    {
    public:
        typedef XDSP::Node::ProcessAttributes
            ProcessAttributes;  // Pass default implementation back to the
                                // template.
    };

    ////////
    // Worker does the processing
    template<class TIOAdapter>
    class Worker : public TIOAdapter
    {
    public:
        ...
        static void ProcessBuffer(const XDSP::ProcessGlobals& process_globals,
                                  TIOAdapter& voices,
                                  const typename Node::ProcessAttributes& pa)
        {
            ...
            {
                bands.x[i][0]
                    = voices.member_gather(my_offsetof(m_bandState[i].x[0]));
                bands.x[i][1]
                    = voices.member_gather(my_offsetof(m_bandState[i].x[1]));
                bands.y[i][0]
                    = voices.member_gather(my_offsetof(m_bandState[i].y[0]));
                bands.y[i][1]
                    = voices.member_gather(my_offsetof(m_bandState[i].y[1]));
                //#pragma unroll(5)
                for (int32 j = 0; j < 5; j++)
                {
                    bands.a[i][j] = voices.member_gather(
                        my_offsetof(m_bandState[i].a[j]));
                }
            }
            ...
            for (int32 t = 0; t < block_length; t++)
            {
                ...
                if (numBands > 0)
                {
                    ...
                    const vf y0 = bs.a[bd][0] * io_sample
                                  + bs.a[bd][1] * bs.x[bd][0]
                                  + bs.a[bd][2] * bs.x[bd][1]
                                  - bs.a[bd][3] * bs.y[bd][0]
                                  - bs.a[bd][4] * bs.y[bd][1];
                ...
                // Fix for aggressive stores (post-checked loop): keep local
                // scope for temporary.
                if (t == block_length - 1)
                {
                    bands_out = bs;
            ...
            for (int32 i = 0; i < numBands; i++)
            {
                voices.member_scatter(bands_out.x[i][0],
                                      my_offsetof(m_bandState[i].x[0]));
                voices.member_scatter(bands_out.x[i][1],
                                      my_offsetof(m_bandState[i].x[1]));
                voices.member_scatter(bands_out.y[i][0],
                                      my_offsetof(m_bandState[i].y[0]));
                voices.member_scatter(bands_out.y[i][1],
                                      my_offsetof(m_bandState[i].y[1]));
            }
        };  // ProcessBuffer()
    };      // class Worker
    ...
    public:
        ...
        vforceinline void
        PreProcessBuffer(const XDSP::ProcessGlobals& process_globals) {};
        template<class TIOAdapter>
        friend class Worker;
